...
import { Match, Team, MatchEventType, UserRole, Player, MatchStatus, TacticalPosition, CurrentUser, MatchMedia } from '../types';
import { 
  Timer, X, Flag, AlertTriangle, ShieldAlert, Goal,
  User, Activity, List, MessageCircle, Send, Lock, CornerUpRight, Play, Square, Eye,
  Video, Image as ImageIcon, Plus
} from 'lucide-react';
...
interface MatchDetailViewProps {
  ...
  homeTeam: Team;
  awayTeam: Team;
  currentUser: CurrentUser; // Updated to use robust User object
  onClose: () => void;
  ...
  onAddMedia: (matchId: string, type: 'IMAGE' | 'VIDEO', url: string) => void;
}

const MatchDetailView: React.FC<MatchDetailViewProps> = ({ 
  match, homeTeam, awayTeam, currentUser, onClose, onAddEvent, onViewPlayer, onSendMessage, onSaveMatchTactics, onAddMedia
}) => {
  const [activeTab, setActiveTab] = useState<'TIMELINE' | 'LINEUPS' | 'STATS' | 'CHAT' | 'TRANSMISSION' | 'MEDIA'>('TIMELINE');
  ...
  const renderLiveConsole = () => {
    // Definindo um estilo base consistente para os botões de ação
    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 p-4 pb-8 z-50 animate-in slide-in-from-bottom shadow-2xl">
        {!consoleOpen ? (
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex flex-col items-center w-1/3">
              ...
              <span className="text-slate-400 text-xs mt-2">{match.arenaId}</span>
            </div>
